...
const onNavToggleClick = () => {
  if (navMain.classList.contains("page-nav--closed")) {
    ...
    navMain.classList.remove("page-nav--opened");
      ...
      const exampleControl = document.querySelector(
        ".example__range-scale-control"
      );

      const onExampleClick = (event) => {
        const imageAfter = exampleSlider.querySelector(
          ".example__image--after"
        );
        const imageBefore = exampleSlider.querySelector(
        ...
        if (event.target.matches(".example__button--after")) {
          ...
          exampleControl.classList.remove(
            "example__range-scale-control--after"
          );
        }
      };
